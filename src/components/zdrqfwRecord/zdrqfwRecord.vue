...
export default {
  data(){
      return {
            ...
            headerList:
                [
                {name:'高血压信息',url:'gxy/2011_gxysf_p.html'},
                {name:"糖尿病信息",url:"tnb/2011_tnbsf_p.html"}
                //{name:"老年人信息",url:"lnr/2011_lnrzytzbs_p.html"}
                //{name:"高危人群信息",url:"lnr/2011_lnrzytzbs_p.html"}
                ],
            headerCkList:[1,0,0],
            index:0,//标注激活的下标
  ...
  created(){
        ...
        this.$store.commit('setPageTitle','重点人群服务');
        //var query = this.$route.query;//拿到archivesCenter页面传参进来的 对象
        //console.log(query);
  ...
  methods:{
        ...
        querySfid(index){
            var params={"daid":"790033279|52012700000000230934|JN"};
            ...
            if(index==1){
                ...
                this.pages=1;
                //查到高血压的 所有 sfid sfrq 
                this.api.getHypertensionSfjlByDaid(params).then(
                    res=>{
                        if(res.code==1&&res.data.length>0){
                            ...
                            this.url=this.baseurl+this.headerList[this.index].url+`?pages=`+this.pages+`&dysflist=`+this.lastArr+`&daid=790033279|52012700000000230934|JN&totals=`+this.totals;
