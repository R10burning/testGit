...
export default {
  data(){
      return {
            ...
            url:'',
            headerList:[{name:'高血压信息',url:'gxy/2011_gxysf_p.html'}], 
            index:0,//标注激活的下标
                ...
                this.api.getHypertensionSfjlByDaid(params).then(
                    res=>{
                        if(res.code==1&&res.data.length>0){
                            ...
                            this.url=this.baseurl+this.headerList[this.index].url+`?pages=`+this.pages+`&dysflist=`+this.lastArr+`&daid=`+this.query.daid+`&totals=`+this.totals;
